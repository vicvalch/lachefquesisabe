...
import {
  ...
  type PublicDemoRegistrationFormValues,
...
import type { DemoMode, PrimaryInterest } from "@/types/database";

function defaultInterestForMode(mode: DemoMode): PrimaryInterest {
  return mode === "virtual" ? "virtual_demo" : "in_person_demo";
}

export function PublicDemoRegistrationForm({
  slug,
  mode,
}: {
  slug: string;
  ...
  const defaultValues: PublicDemoRegistrationFormValues = {
    name: "",
    phone: "",
    email: "",
    primary_interest: defaultInterestForMode(mode),
    message: "",
  ...
  } = useForm<PublicDemoRegistrationFormValues>({
    resolver: zodResolver(publicDemoRegistrationFormSchema),
    defaultValues,
  });

  async function onSubmit(values: PublicDemoRegistrationFormValues) {
    setServerError(null);
    try {
      const response = await fetch(`/api/demos/${slug}/register`, {
        method: "POST",
      ...
      if (!response.ok || !result.ok) {
        setServerError(
          result.error || "No pudimos guardar tu registro. Intenta de nuevo.",
        );
        return;
      }

      reset(defaultValues);
      router.push(`/gracias?demo=${slug}`);
    } catch {
    ...
    >
      ...
      <div className="sm:col-span-2">
        <label className="flex items-start gap-3 text-sm text-ink-soft">
          ...
          />
          <span>
            Acepto que La Chef que Sí Sabe me contacte para confirmar mi
            lugar en esta demostración. Consulta nuestra{" "}
            <a
              href="/privacidad"
              target="_blank"
